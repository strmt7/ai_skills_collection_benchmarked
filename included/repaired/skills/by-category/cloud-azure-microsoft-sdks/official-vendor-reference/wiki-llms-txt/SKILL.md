...
- {Page Title} (unavailable local link target: `{relative-path-to-md}`): {One-sentence description of what the reader will learn}

## Optional

- {Page Title} (unavailable local link target: `{relative-path-to-md}`): {Description — these can be skipped for shorter context}
```
...
4. **H2 sections** — Organized by topic, each with a list of `Title (unavailable local link target: `url`): Description` entries
...
- **Preserve citations** — all `file:line (unavailable local link target: `URL`)` links stay as-is
...
The root `./llms.txt` links into `wiki/` (e.g., `Guide (unavailable local link target: `./wiki/onboarding/contributor-guide.md`)`). The `wiki/llms.txt` uses wiki-relative paths (e.g., `Guide (unavailable local link target: `./onboarding/contributor-guide.md`)`).
