...
| Parent-relative source links | `text (unavailable local link target: `../../src/file.cs`)` | Broken (wiki is separate) | Convert to plain text |
...
Convert `text (unavailable local link target: `../path`)` to plain text. Preserves same-directory `.md` links and external URLs.
...
- **Linked citations** (`file:line (unavailable local link target: `URL`)`) are standard markdown — preserve them as-is
