...
- ALWAYS cite files using the resolved citation format (linked for remote repos, local otherwise): `file_path:line_number (unavailable local link target: `REPO_URL/blob/BRANCH/file_path#Lline_number`)` or `(file_path:line_number)`
