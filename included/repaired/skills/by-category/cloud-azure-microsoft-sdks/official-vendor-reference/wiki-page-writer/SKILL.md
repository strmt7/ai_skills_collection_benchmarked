...
2. **Ask the user**: _"Is this a local-only repository, or do you have a source repository URL (e.g., GitHub, Azure DevOps)?"_
   - Remote URL provided → store as `REPO_URL`, use **linked citations**: `file:line (unavailable local link target: `REPO_URL/blob/BRANCH/file#Lline`)`
...
- Every non-trivial claim needs a citation with the resolved format:
  - **Remote repo**: `src/path/file.ts:42 (unavailable local link target: `REPO_URL/blob/BRANCH/src/path/file.ts#L42`)`
  - **Local repo**: `(src/path/file.ts:42)`
  - **Line ranges**: `src/path/file.ts:42-58 (unavailable local link target: `REPO_URL/blob/BRANCH/src/path/file.ts#L42-L58`)`
...
- **Inline links**: When mentioning a concept, component, or pattern covered on another wiki page, link to it inline using relative Markdown links: `Component Name (unavailable local link target: `../NN-section/page-name.md`)` or `Section Title (unavailable local link target: `../NN-section/page-name.md#heading-anchor`)`
- **Related Pages section**: End every page with a "Related Pages" section listing connected wiki pages:
  ...
  | Page | Relationship |
  |------|-------------|
  | Authentication (unavailable local link target: `../02-architecture/authentication.md`) | Handles token validation used by this API |
  | Data Models (unavailable local link target: `../03-data-layer/models.md`) | Defines the entities processed here |
  | Contributor Guide (unavailable local link target: `../onboarding/contributor-guide.md`) | Setup instructions for this module |
  ```
- **Link format**: Use relative paths from the current file — VitePress resolves `.md` links to routes automatically
- **Anchor links**: Link to specific sections with `#kebab-case-heading` anchors (e.g., `error handling (unavailable local link target: `../02-architecture/overview.md#error-handling`)`)
