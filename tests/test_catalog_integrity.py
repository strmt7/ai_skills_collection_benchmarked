...
def test_priority_repositories_present():
...
def test_priority_manifest_matches_mirrors():
    catalog = load("data/skills_catalog.json")
    manifest = load("included/priority/manifest.json")
    priority_ids = {entry["id"] for entry in catalog if entry["source_tier"].startswith("priority")}
    assert {entry["id"] for entry in manifest} == priority_ids
...
def test_static_benchmark_results_are_real_and_current():
    ...
    assert results["summary"]["runtime_artifacts_recorded"] == 0
    assert all(track["runtime_artifacts_recorded"] == 0 for track in results["tracks"])
