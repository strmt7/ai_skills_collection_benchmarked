...
def _load_canonical_repo_metadata(payload: dict[str, object]) -> RepoMetadata | None:
    ...
    if len(present) != len(canonical_keys):
        ...
        raise RuntimeError(f"{CANONICAL_METADATA_PATH} has partial repository metadata: {missing}")
...
def load_metadata(repo_root: Path) -> BadgeMetadata:
    payload = json.loads((repo_root / CANONICAL_METADATA_PATH).read_text(encoding="utf-8"))
...
def _git_executable() -> str:
    ...
    if git_bin is None:
        raise RuntimeError("git executable is required to resolve README badge metadata.")
    return git_bin
...
def _resolve_remote_name(repo_root: Path) -> str:
    ...
    try:
        configured_remote = _run_git(repo_root, "config", "--get", f"branch.{branch_name}.remote")
    except subprocess.CalledProcessError:
        configured_remote = ""
...
def resolve_repo_metadata(repo_root: Path) -> RepoMetadata:
    ...
    host, owner, repo = _parse_remote_url(_run_git(repo_root, "remote", "get-url", remote_name))
